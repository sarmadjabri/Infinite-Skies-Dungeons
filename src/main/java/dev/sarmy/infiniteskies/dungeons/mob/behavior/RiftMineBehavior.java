...
public final class RiftMineBehavior implements MobBehavior {
 ...
 @Override public void applyEquipment(LivingEntity mine){mine.setAI(false);}
 @Override public void tick(LivingEntity mine,long tick){
  Long fuse=fuses.get(mine.getUniqueId());
  if(fuse==null){ for(Player player:mine.getWorld().getPlayers()) if(!player.isDead()&&player.getLocation().distanceSquared(mine.getLocation())<=12.25){fuses.put(mine.getUniqueId(),tick+TELEGRAPH_TICKS);mine.getWorld().playSound(mine.getLocation(),Sound.BLOCK_NOTE_BLOCK_HAT,1f,.7f);return;} return; }
